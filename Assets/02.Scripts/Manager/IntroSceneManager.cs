...
/* 작성일 : 2025.06.16
   ...
   인트로 씬의 진행을 담당하는 로직 스크립트 
   
   처음 주인공의 dialogue ( 독백과 비슷 ) 
   =>
   메인 인트로 씬 ( ShowSubtitle을 통해서 대사 출력 )
   => 
   ...
   메인 인트로 씬 내에서의 출력과 dialouge를 출력하는 메서드가 서로 다름
...
public class IntroManager : MonoBehaviour
{
    ...
    public string[] mainSceneDialogues;
    ...
    {
        currentDialogues = Const.introDialogues; 
        mainSceneDialogues = Const.introMainDialogues;
    ...
    public void NextDialogueInit() // 마지막 다이얼로그를 출력하기 전 초기화
    {
        ...
        currentDialogues = Const.introEndDialogues; 
        index = 0;
        dialogueText.text = string.Empty;
        indicatorText.alpha = 0f;
    }

    public void LoadMainGameScene() // 메인 게임씬을 로드하는 메서드
    {
        SceneManager.LoadScene(Const.MainGameScene);
    }
    ...
    /* 처음에 나오는 독백과 다르게 타임라인 도중 대사를 출력하는 메서드
       Signal를 통해서 하나씩 출력, 만약 출력할 대사가 없다면 timeline 종료*/
    #region [Subtitle 출력]
    public void ShowSubtitle()
    {
        ...
        if (Const.introMainDialogues.Length > subtitleIndex)
        {
            subtitleCoroutine = StartCoroutine(ShowSubtitleCoroutine(Const.introMainDialogues[subtitleIndex], 5f));
        }
        else
        {
            TurnOffisPlaying();
